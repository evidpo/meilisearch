...
### ⛔ СТОП! Перед тем как сказать "Готово" — сделай коммит!

**КАЖДАЯ завершённая задача = коммит + пуш. БЕЗ ИСКЛЮЧЕНИЙ.**

```bash
# Последовательность ВСЕГДА:
git add -A
git commit -m "feat/fix/docs(scope): описание на русском
...
| Исправил баг | ✅ ДА |
| Добавил функцию | ✅ ДА |
| Обновил документацию | ✅ ДА |
| Изменил настройки | ✅ ДА |
| Создал changelog | ✅ ДА |
| Только прочитал файлы | ❌ НЕТ |

### Чеклист перед "Готово":

```
⬜ Есть изменённые файлы? → git status
⬜ Если да → git add -A && git commit && git push
⬜ Только потом говорю "Готово"
```
...
✅ Коммитить сразу после любого изменения или завершения задачи
✅ Пушить сразу после коммита (git push)
